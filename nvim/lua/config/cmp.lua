-- The cmp/LuaSnip stack. Held off 'runtimepath' by `on = {}` in plug.lua and
-- required from config/completion.lua on first insert/cmdline entry.
vim.fn['plug#load'](
	'nvim-cmp',
	'cmp-buffer',
	'cmp-path',
	...
	sources = cmp.config.sources({
		...
		{ name = 'path', priority = 500 },
		{
			...
			option = {
				...
				-- netrw/terminal leave behind, and re-indexes the lot
				-- on every keystroke.
				get_bufnrs = function()
					local bufs = {}
...
-- symbols in scope. Searches getcwd(), so rooter sets the scope. Costs a brief
-- prefix wait on a bare <C-x> before native <C-x><C-f> and friends.
